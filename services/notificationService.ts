...
export const notificationService = {
    ...
    async scheduleTaskNotifications(task: Task): Promise<string[]> {
        ...
        // Skip deleted tasks
        if (task.deletedAt) {
            console.log('Skipping notifications for deleted task:', task.id);
            return [];
        }
        
        // Skip completed non-recurring tasks
        if (task.completed && (!task.recurrence || task.recurrence === 'none')) {
            console.log('Skipping notifications for completed non-recurring task:', task.id);
            return [];
        }
    ...
    async cancelTaskNotifications(notificationIds?: string[]) {
